...
const ResponsiveSidebar = ({showSidebar, setShowSidebar}) => {
    ...
    useEffect(() => {
            ...
            entries.forEach(entry => {
                ...
                switch (entry.target.id) {
                    ...
                    case 'skills':
                        ...
                        break;
                    // Since the projects section is longer than the other,
                    // at certain points the threshold might be met for both
                    // the projects and education sections.  To prevent both 
                    // links from turning white at same time, extra variables
                    // are used (the React state cannot be used for this bc
                    // it is not updated immediately d/t async nature)
                    case 'projects':
        ...
        const projectObserver 
            = new IntersectionObserver(ioCallback, ioOptionsForProjectEl);
    ...
    return (
        ...
        justify-center gap-6 px-3 pb-3 left-0 top-14 w-screen transform
        transition duration-200 ease-in-out ${showSidebar ? 'fixed' : '-translate-y-66 absolute'}
        lg:inset-y-0 lg:w-64 lg:items-center lg:px-0 lg:pb-0 lg:translate-y-0 lg:fixed`}
        >
            <img className="hidden lg:block" src={profilePic} 
            alt="Matthew Di Micelli profile pic" />
            <nav>
